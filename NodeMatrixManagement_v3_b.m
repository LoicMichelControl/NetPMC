    ...
    function [WiringSortList, WiringSubList, CommonNeuron_sort, CommonWiring_sort, NeuronWeigthList, WiringSubList_aux, newConnectedID_vec] = ...
        NodeMatrixManagement_v3_b(NeuronWeigthList, newConnectedID, newConnectedID_vec,  EnableCoupling, ...
    ...
    WiringRank_bis = WiringRank;

    newConnectedID_vec = [newConnectedID_vec, newConnectedID];

    for (iii = 1: length(ForbConnexionNeuron_A) )

         count_Wiring = 1;
    
        % Sweep all lines of WiringRank
        for uu = 1: length(WiringRank_bis)
            ...
            if ( find(WiringRank_bis(uu,1) == ListConnectedNeuron) & find(WiringRank_bis(uu,2) == ListConnectedNeuron ) & ...
                    ~(WiringRank_bis(uu,1) == ForbConnexionNeuron_A(iii) & WiringRank_bis(uu,2) == ForbConnexionNeuron_B(iii) ) & ...
                    ~(WiringRank_bis(uu,1) == ForbConnexionNeuron_B(iii) & WiringRank_bis(uu,2) == ForbConnexionNeuron_A(iii) ))
        
                WiringSubList(count_Wiring,:) = [ WiringRank_bis(uu,1), WiringRank_bis(uu,2), WiringRank_bis(uu,3) ];
        ...
        WiringSubList_aux = WiringSubList;

              clear WiringRank_bis
            WiringRank_bis = WiringSubList;
   ...
   if ( isempty( ForbConnexionNeuron_C ) == 0 & isempty( ForbConnexionNeuron_D ) == 0  ) 

       for (iii = 1: length(ForbConnexionNeuron_C) )
            ...
            for uu = 1: length(WiringRank_bis)
                ...
                % neurons by the #wire index
                if ( find(WiringRank_bis(uu,1) == ListConnectedNeuron) & find(WiringRank_bis(uu,2) == ListConnectedNeuron ) & ...
                        ~(WiringRank_bis(uu,1) == ForbConnexionNeuron_C(iii) & WiringRank_bis(uu,2) == ForbConnexionNeuron_D(iii) ) & ...
                        ~(WiringRank_bis(uu,1) == ForbConnexionNeuron_D(iii) & WiringRank_bis(uu,2) == ForbConnexionNeuron_C(iii) ))
    ...
    WiringSubList = WiringRank_bis;

    %%%%%%%%% Deprecated since the procedure becomes iterative from v3 
    %
    %
    % count_Wiring = 1;
    % 
    ...
    %     % neurons by the #wire index
    %     if ( find(WiringRank_bis(uu,1) == ListConnectedNeuron) & find(WiringRank_bis(uu,2) == ListConnectedNeuron ) & ...
    %             ~(WiringRank_bis(uu,1) == ForbConnexionNeuron_C & WiringRank_bis(uu,2) == ForbConnexionNeuron_D ) & ...
    %             ~(WiringRank_bis(uu,1) == ForbConnexionNeuron_D & WiringRank_bis(uu,2) == ForbConnexionNeuron_C ))
    % 
    %         WiringSubList(count_Wiring,:) = [ WiringRank_bis(uu,1), WiringRank_bis(uu,2), WiringRank_bis(uu,3) ];
    % 
    %         count_Wiring = count_Wiring + 1;
    ...
    for ii = 1:length( WiringSubList(:,1) )
        ...
        for jj = 1:2 % sweep the two first columns
          ...
          %  for id_new = 1:length( newConnectedID_vec )
                ...
                for iii = 1:length( WiringSubList(:,1) )
                    ...
                    if ( isempty( find( Pivot == WiringSubList(iii, 1:2 ) ) ) == 0 )
                        ...
                        % present in the WiringSubList -> then put a (-) to the
                        ...
                        % if ( ( EnableCoupling == 1 & ( find ( WiringSubList(iii, 3-find ( Pivot == WiringSubList(iii,[1:2]) ) ) == newConnectedID_vec(id_new) ) ) == 1 ) )
                        %     CommonNeuron(qq, kk) = -WiringSubList(iii, 3-find ( Pivot == WiringSubList(iii,[1:2]) ) );
                        % else
                        % 
                        % end    
    ...
    for aaa = 1:length(  CommonNeuron_sort_(:,1)  )
      ...
      % use here an extra variable because the length can be different 
      % -> may create error with 'CommonNeuron_sort'
      CommonNeuron_sort_vec = [CommonNeuron_sort_(aaa,1), unique(CommonNeuron_sort_(aaa,2:end), 'stable')];
    
        CommonNeuron_sort(aaa,1:length( CommonNeuron_sort_vec ) ) = CommonNeuron_sort_vec;
            ...
            for zz = 1: length( WiringSubList(:,1)  )
                ...
                if ( WiringSubList(zz,1) == Pivot & abs(CommonWiring_sort( ii, jj )) == WiringSubList(zz,2) & valid == 0 )
                    ...
                    if ( CommonWiring_sort( ii, jj ) < 0)
                        ...
                        % Notify the exclusion of the rejected wire (not
                        % here - just keep as checking point)
